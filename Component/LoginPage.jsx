...
class LoginPage extends Component {
  ...
  onSubmitForm = (e) => {
    ...
    //ajax 요청으로 url 변경 없이 서버로 로그인 요청
    const xhr = new XMLHttpRequest();
    xhr.open('get', `/login?id=${this.state.id}&pw=${this.state.pw}`);
    xhr.send();
    xhr.onload = function () {
      //console.log(xhr.response)
      if(xhr.response === "ID_FALSE") {
        document.querySelector('#id').value =""
        document.querySelector('#password').value =""
        alert('아이디 정보가 없습니다.');
      }
      else if(xhr.response ==="PW_FALSE") {
        document.querySelector('#id').value =""
        document.querySelector('#password').value =""
        alert('비밀번호가 틀립니다.');
      }
        ...
        //로그인 성공처리
        //console.log(JSON.parse(xhr.response)); // DB로 부터 사용자 정보 가져옴->리덕스로 사용자 정보 관리
        //로그인 버튼 -> 로그아웃 버튼으로 변경 (리덕스로 로그인 버튼 컴포넌트 props에 접근해야할거같음 )
        //회원가입 버튼 -> 숨기기 -> 리덕스
    ...
    success: function(obj) {
      ...
      window.Kakao.API.request({
        ...
        success: res=> {
          ...
          xhr.send();
          xhr.onload = () => {
            //아래 xhr.reponse가 로그인한 사용자 payload임 이 값 -> 리덕스
            var userInfor = JSON.parse(xhr.response);
            store.dispatch({type:"LOGIN", name:userInfor.name, position: userInfor.position, user_id:userInfor.user_id, password: userInfor.password})
            store.dispatch({type:"HOME"})
          }
...
changePw = (e) => {
  ...
  render() {
    return (
        <article id = "artiicle_login_component">
          <center><h2 id="label">로그인</h2></center>
          <form onSubmit ={this.onSubmitForm}>
            <center><input type="text" placeholder ="ID" id = "id" onChange={this.changeId}></input></center>
            <center><input type ="password" placeholder ="Password" id ="password" onChange={this.changePw}></input></center>
            <center><input type ="submit" id ="go_to_login" value="login"></input></center>
          </form>
          <hr id ="hr1"></hr>
